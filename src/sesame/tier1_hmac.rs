...
// Spec: ANSI/SCTE 130-9 (SESAME) draft v0.4 §8.2.
//
...
use crate::sesame::message::{hex_decode, hex_encode, SesameError};
...
pub fn verify_any(keys: &[Vec<u8>], canonical: &str, provided_hex: &str) -> Result<(), SesameError> {
...
mod tests {
    ...
    fn wrong_key_rejected() {
        let sig = sign(b"k1", "c");
        assert_eq!(verify(b"k2", "c", &sig), Err(SesameError::SignatureMismatch));
    }
