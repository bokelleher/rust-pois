...
use crate::sesame::keys::KeyProvider;
use crate::sesame::message::SesameError;
...
mod tests {
    use super::*;
    use crate::sesame::keys::{ChannelScope, HmacKey, StaticKeyProvider};
    ...
    #[test]
    fn parse_scope() {
        assert_eq!(parse_scope_channel("channel=SportsFeed-East"), Some("SportsFeed-East"));
    ...
    fn authorized_channel_accepted() {
        let p = provider();
        assert_eq!(
            authorize(&p, "sas-east-01", "channel=SportsFeed-East", Some("SportsFeed-East")),
    ...
    fn unauthorized_channel_denied() {
        let p = provider();
        assert_eq!(
            authorize(&p, "sas-east-01", "channel=PremiumFeed", Some("PremiumFeed")),
            Err(SesameError::ScopeDenied)
        );
    }
    ...
    fn scope_target_mismatch_denied() {
        ...
        let p = provider();
        assert_eq!(
            authorize(&p, "sas-east-01", "channel=SportsFeed-East", Some("PremiumFeed")),
            Err(SesameError::ScopeDenied)
        );
    }
}
