// src/sesame/mod.rs
//
// SESAME — Secure ESAM Authentication and Message Encryption.
// Framework-agnostic core implementing the three additive security tiers of
// ANSI/SCTE 130-9 (SESAME) draft v0.4, bidirectionally (verify inbound
// requests, sign/encrypt outbound responses).
//
// This module has NO HTTP-framework dependency. The axum adapter lives in
// `crate::sesame::axum_glue` (binary-side); benches and tests use this core
// directly. Points where the implementation resolves a gap or contradiction in
// the draft are tagged `[BO]` and recorded in docs/SESAME_reconciliation.md.
...
pub use message::{SesameError, SesameHeaders};

use keys::KeyProvider;
use message::{hex_decode, hex_encode, PROTOCOL_VERSION};
...
pub fn verify_request(
    ...
    let plaintext = if headers.encrypted {
        let enc_key_id = headers.enc_key_id.as_deref().ok_or(SesameError::DecryptFailed)?;
        ...
        let aead = provider.aead_key(enc_key_id).ok_or(SesameError::DecryptFailed)?;
...
pub fn sign_response(
    ...
    let timestamp = now.format(&Rfc3339).map_err(|_| SesameError::ExpiredTimestamp)?;
    ...
    let canonical =
        canonical::response_canonical(params.correlation, &timestamp, &nonce, &body_hash, params.scope);
...
mod tests {
    use super::*;
    use keys::{AeadKey, ChannelScope, HmacKey, StaticKeyProvider};
    use replay::InMemoryReplayCache;
    use tier3_aead::KEY_LEN;
    ...
    fn provider() -> StaticKeyProvider {
        StaticKeyProvider::new()
            ...
            .with_signing_key("pois-primary", HmacKey(b"pois-secret".to_vec()), ChannelScope::all())
    ...
    fn make_request(tier: Tier, encrypt_with: Option<&str>) -> (SesameHeaders, Vec<u8>) {
        ...
        let scope = if tier >= Tier::Two { Some("channel=SportsFeed-East") } else { None };
        ...
        let (body, enc_headers) = if tier >= Tier::Three {
            ...
            let aad =
                tier3_aead::aad_for_headers(PROTOCOL_VERSION, "sas-east-01", timestamp, nonce, scope);
            let ct = tier3_aead::seal(&aead.0, &iv, &aad, XML).unwrap();
            (
                ct,
                Some((enc_key_id.to_string(), hex_encode(&iv))),
            )
        } else {
            (XML.to_vec(), None)
        };
        ...
        let body_hash = canonical::body_hash_hex(&body);
        let canonical =
            canonical::request_canonical("POST", "/esam?channel=SportsFeed-East", timestamp, nonce, &body_hash, scope);
    ...
    fn tier1_roundtrip() {
        ...
        let v = verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::One)
    ...
    fn tier2_roundtrip() {
        ...
        let cache = InMemoryReplayCache::new(300);
        let v = verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::Two)
    ...
    fn tier3_roundtrip_decrypts_to_original_xml() {
        ...
        let cache = InMemoryReplayCache::new(300);
        let v = verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::Three)
    ...
    fn tier0_passthrough_when_allowed() {
        ...
        let v = verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, XML, now(), Tier::Zero)
    ...
    fn tier0_rejected_when_tier1_required() {
        ...
        let h = SesameHeaders::default();
        assert_eq!(
            verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, XML, now(), Tier::One)
                .err(),
    ...
    fn tampered_body_rejected() {
        ...
        let cache = InMemoryReplayCache::new(300);
        assert_eq!(
            verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::One).err(),
    ...
    fn replayed_nonce_rejected() {
        let (h, body) = make_request(Tier::One, None);
        let cache = InMemoryReplayCache::new(300);
        assert!(verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::One).is_ok());
    ...
    fn stale_timestamp_rejected() {
        ...
        assert_eq!(
            verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, later, Tier::One).err(),
    ...
    fn unauthorized_channel_rejected() {
        ...
        let canonical = canonical::request_canonical("POST", "/esam?channel=PremiumFeed", timestamp, nonce, &body_hash, Some(scope));
        let signature = tier1_hmac::sign(&key, &canonical);
        let h = SesameHeaders {
            version: Some(PROTOCOL_VERSION.to_string()),
            key_id: Some("sas-east-01".to_string()),
        ...
        let ctx = RequestContext { method: "POST", path: "/esam?channel=PremiumFeed", target_channel: Some("PremiumFeed") };
        let cache = InMemoryReplayCache::new(300);
        assert_eq!(
            verify_request(&SesameConfig::default(), &p, &cache, &ctx, &h, XML, now(), Tier::Two).err(),
    ...
    fn truncated_gcm_tag_rejected() {
        ...
        let cache = InMemoryReplayCache::new(300);
        let err = verify_request(&SesameConfig::default(), &provider(), &cache, &ctx(), &h, &body, now(), Tier::Three).err();
    ...
    fn response_sign_and_client_verify_roundtrip() {
        ...
        let canonical = canonical::response_canonical("sig-20260224-001", &ts, &nonce, &body_hash, Some("channel=SportsFeed-East"));
    ...
    fn response_iv_differs_from_request_iv() {
        ...
        let resp_iv = resp
            ...
            .map(|(_, v)| v.clone())
            .unwrap();
        assert_ne!(req_iv, resp_iv, "response reused the request IV under the same EncKeyId");
    }
    ...
    fn tier3_response_uses_fresh_iv() {
        ...
        let iv = |r: &SignedResponse| r.headers.iter().find(|(k, _)| *k == message::H_IV).map(|(_, v)| v.clone()).unwrap();
